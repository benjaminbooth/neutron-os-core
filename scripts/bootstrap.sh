...
# Save current shell options so we can restore them (important when sourced!)
# This prevents 'set -euo' from affecting the parent shell
_BOOTSTRAP_OLD_OPTS=$(set +o)
_BOOTSTRAP_OLD_SHOPT=""
if [[ -n "${BASH_VERSION:-}" ]]; then
    _BOOTSTRAP_OLD_SHOPT=$(shopt -p 2>/dev/null || true)
fi
...
_bootstrap_cleanup() {
    ...
    eval "$_BOOTSTRAP_OLD_OPTS" 2>/dev/null || true
    if [[ -n "${_BOOTSTRAP_OLD_SHOPT:-}" ]]; then
        eval "$_BOOTSTRAP_OLD_SHOPT" 2>/dev/null || true
    fi
    unset _BOOTSTRAP_SOURCED _SCRIPT_PATH _CURRENT_SHELL _BOOTSTRAP_OLD_OPTS _BOOTSTRAP_OLD_SHOPT 2>/dev/null || true
}
...
# Silence direnv's verbose output for this session (prevents noise after bootstrap)
# Must be set early, before direnv hook runs on prompt return
...
if command -v register-python-argcomplete &>/dev/null; then
    eval "$(register-python-argcomplete neut)" 2>/dev/null || true
    echo "✓ Tab completion activated for this session"
    ...
    if [[ -n "$shell_config" ]] && [[ -f "$shell_config" ]]; then
        if ! grep -q 'register-python-argcomplete neut' "$shell_config" 2>/dev/null; then
            echo '' >> "$shell_config"
            echo '# neut CLI tab completion' >> "$shell_config"
            echo 'eval "$(register-python-argcomplete neut)"' >> "$shell_config"
...
if command -v direnv &>/dev/null; then
    ...
    if [[ -f "$PROJECT_ROOT/.envrc" ]]; then
        # Silence direnv's verbose output during bootstrap
...
echo "  neut sense corrections      # Review voice memos"
echo "  neut doc publish <file>     # Publish a document"
echo ""
...
# ═══════════════════════════════════════════════════════════════════════════
# Cleanup: Restore shell options (critical when sourced!)
# ═══════════════════════════════════════════════════════════════════════════
# Without this, 'set -euo pipefail' persists in the parent shell and can cause
# crashes when other tools (like VS Code's terminal integration) reference
# unset variables.
