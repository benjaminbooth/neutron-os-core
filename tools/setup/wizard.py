...
from tools.setup.probe import ProbeResult, run_probe
from tools.setup.state import SetupState, load_state, save_state
from tools.setup.tester import ChannelTester
...
class SetupWizard:
    ...
    def __init__(self, root: Optional[Path] = None):
        if root is None:
            from tools.setup.probe import _find_project_root
    ...
    def _phase_config(self) -> None:
        ...
        facility_exists = (self.root / "tools" / "agents" / "config" / "facility.toml").exists()
        models_exists = (self.root / "tools" / "agents" / "config" / "models.toml").exists()
    ...
    def _generate_facility_toml(self, name: str, ftype: str) -> None:
        ...
        dest = self.root / "tools" / "agents" / "config" / "facility.toml"
        if dest.exists():
            renderer.info("facility.toml already exists — keeping current version")
            self.state.config_files_created["facility.toml"] = True
            return

        template = self.root / "tools" / "agents" / "config.example" / "facility.toml"
    ...
    def _generate_models_toml(self) -> None:
        ...
        dest = self.root / "tools" / "agents" / "config" / "models.toml"
        if dest.exists():
            renderer.info("models.toml already exists — keeping current version")
            self.state.config_files_created["models.toml"] = True
            return

        template = self.root / "tools" / "agents" / "config.example" / "models.toml"
        if not template.exists():
    ...
    def fix(self, name: str) -> None:
        ...
        from tools.setup.guides import get_guide
