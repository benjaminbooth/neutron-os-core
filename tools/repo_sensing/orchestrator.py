...
Or used as a library:
    from tools.repo_sensing.orchestrator import RepoExportOrchestrator
    orch = RepoExportOrchestrator()
    orch.run(days=90, output_dir=Path("tools/exports"))
"""
...
from tools.repo_sensing.base import RepoSourceProvider, is_within_days
from tools.repo_sensing.config import SourceConfig, load_config
...
def _create_provider(source: SourceConfig) -> RepoSourceProvider:
    ...
    if source.provider == "gitlab":
        from tools.repo_sensing.providers.gitlab import GitLabProvider
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Directory for output file (default: tools/exports)",
    )
    parser.add_argument(
        "--days",
        type=int,
