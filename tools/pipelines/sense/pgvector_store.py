...
Quick Start (local):
    ...
    # Use it
    from tools.pipelines.sense.pgvector_store import VectorDB
    db = VectorDB()
...
class PgVectorStore:
    ...
    def chunks(self) -> list:
        ...
        from tools.pipelines.sense.signal_rag import SignalChunk
        return [
            SignalChunk(
                signal_id=r.id,
                text=r.text,
...
def k3d_up() -> bool:
    ...
    print()
    print("To use:")
    print("  from tools.pipelines.sense.pgvector_store import VectorDB")
    print("  db = VectorDB()")
    print()
