...
from tools.pipelines.sense.bootstrap import (
    Bootstrap,
    BootstrapConfig,
    BootstrapStep,
    StepResult,
...
class TestMigrations:
    ...
    def test_migrations_up_to_date_skips(self, bootstrap):
        ...
        with patch.dict(os.environ, {"NEUT_DB_URL": bootstrap.config.db_url}):
            with patch("tools.pipelines.sense.migrations.check_migrations") as mock_check:
    ...
    def test_runs_pending_migrations(self, bootstrap):
        ...
        with patch.dict(os.environ, {"NEUT_DB_URL": bootstrap.config.db_url}):
            with patch("tools.pipelines.sense.migrations.check_migrations") as mock_check:
                ...
                with patch("tools.pipelines.sense.migrations.run_migrations") as mock_run:
    ...
    def test_multiple_runs_safe(self, bootstrap):
        ...
        with patch.object(bootstrap, "check_k3d_cluster") as mock_k3d:
            ...
            with patch.object(bootstrap, "check_postgres") as mock_pg:
                ...
                with patch.dict(os.environ, {"NEUT_DB_URL": bootstrap.config.db_url}):
                    with patch("tools.pipelines.sense.migrations.check_migrations") as mock_mig:
...
class TestBootstrapCLI:
    ...
    def test_check_flag(self):
        ...
        from tools.pipelines.sense.bootstrap import main
