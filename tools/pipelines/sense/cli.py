        ...
        if match and self._valid_subcommands:
            ...
            if suggestions:
                ...
                if (len(suggestions) == 1
                    ...
                    try:
                        from tools.setup.renderer import prompt_yn
...
INBOX_ARCHIVED = _AGENTS_DIR / "inbox" / "archived"  # Processed raw files move here
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    # Config
    from tools.pipelines.sense.correlator import CONFIG_DIR, CONFIG_EXAMPLE_DIR
...
def _ingest_prd_comments(correlator) -> list:
    ...
    from tools.pipelines.sense.extractors.prd_comments import PRDCommentsExtractor
...
def cmd_serve(args: argparse.Namespace) -> None:
    ...
    from tools.pipelines.sense.serve import run_server
...
def cmd_correct(args: argparse.Namespace) -> None:
    ...
    from tools.pipelines.sense.corrector import correct_transcript

    print()
    print("neut sense — transcript correction")
    print("-" * 40)
    ...
    if args.path:
        ...
        if not transcripts:
            print("  No transcripts found. Run 'neut sense ingest --source voice' first.")
            return
...
def cmd_routes(args: argparse.Namespace) -> None:
    ...
    from tools.pipelines.sense.router import Router
    ...
    print("  ╰──────────────────────────────────────────────────────┤")
    print("\n  Commands:")
    print("    neut sense routes --pending     Show queued signals")
    print("    neut sense routes --deliver     Push to endpoints")
    print("    neut sense routes --endpoint X  Show endpoint details")
    print()


def cmd_suggest(args: argparse.Namespace) -> None:
    """LLM-powered signal-to-PRD relevance matching."""
    from tools.pipelines.sense.smart_router import SmartRouter
    ...
    if args.run:
        ...
        if not all_signals:
            print("  No signals found. Run 'neut sense ingest' first.")
            return
    ...
    print("\n  Commands:")
    print("    neut sense suggest --run                 Run LLM matching")
    print("    neut sense suggest --prd <id>            Filter by PRD")
    print("    neut sense suggest --accept sig:prd      Accept suggestion")
    print("    neut sense suggest --reject sig:prd      Reject suggestion")
    print()
...
def cmd_brief(args: argparse.Namespace) -> None:
    ...
    if args.index:
        ...
        try:
            from tools.pipelines.sense.signal_rag import reindex_all_signals
    ...
    if args.topics:
        ...
        print("  ╰──────────────────────────────────────────────────────╯")
        print()
        print("  Usage: neut sense brief \"Kevin\"")
        print("         neut sense brief \"blockers\"")
        print("         neut sense brief \"TRIGA digital twin\"")
        print()
        return
...
def main():
    """CLI entry point for neut sense."""
    parser = get_parser()
    ...
    try:
        ...
        import traceback
        from tools.infra.self_heal import attempt_recovery, emit_cli_error
        ...
        try:
            from tools.infra.subscribers.gitlab_issues import gitlab_issue_handler
...
def _tag_speaker_voice(audio_path: str, start_time: float, end_time: float, agents_dir: "Path") -> None:
    ...
    try:
        from tools.pipelines.sense.voice_id import VoiceProfileStore, SpeakerIdentifier
    except ImportError:
...
def _run_interactive_guided_review(
    ...
    if remaining > 0:
        ...
        print("Run: neut sense corrections --guided")
...
def cmd_corrections(args):
    ...
    from tools.pipelines.sense.correction_review import (
...
def cmd_sources(args):
    ...
    from tools.pipelines.sense.registry import (
        get_registry,
        print_sources_table,
        print_inbox_status,
    )
    ...
    if args.fetch:
        ...
        print(f"  (fetch logic depends on extractor - use `neut sense ingest --source {source_name}`)")
        return
...
def cmd_providers(args):
    ...
    try:
        from tools.pipelines.sense.extractors.docflow_providers import (
            ProviderRegistry,
            sync_all_folders,
        )
    except ImportError as e:
...
def cmd_db(args):
    ...
    import os
    from tools.pipelines.sense.pgvector_store import (
        VectorDB,
        DEFAULT_LOCAL_URL,
        k3d_up,
        k3d_down,
    ...
    if action == "stats":
        try:
            ...
            print("\nIs the database running? Try: neut sense db up")
        ...
        from tools.pipelines.sense.signal_rag import SignalRAG, INDEX_PATH, EMBEDDINGS_PATH
        ...
        try:
            ...
            if rag.backend_name != "pgvector":
                ...
                print("Make sure database is running: neut sense db up")
                return

            # Load file-based data and re-index
            from tools.pipelines.sense.signal_rag import VectorStore
            ...
            print("\nMake sure database is running: neut sense db up")
        ...
        if cmd == "check":
            ...
            if not status.get("connected"):
                ...
                print("   Is the database running? Try: neut sense db up")
                return
            ...
            if status.get("up_to_date"):
                ...
                for rev in status.get("pending_revisions", []):
                    print(f"   - {rev}")
                print("\nRun: neut sense db migrate upgrade head")
            ...
            if not message:
                ...
                print("Example: neut sense db migrate revision -m 'add user table'")
                return
            ...
            print("Usage: neut sense db migrate <command> [revision]")
            print()
            print("Commands:")
            ...
            print()
            print("Examples:")
            print("  neut sense db migrate check")
            print("  neut sense db migrate upgrade head")
            print("  neut sense db migrate downgrade -1")
            print("  neut sense db migrate revision -m 'add user preferences' --autogenerate")
        ...
        print("\n--- Environments ---")
        print("  Local:      neut sense db up  (K3D)")
        ...
        if not args.confirm:
            ...
            print("\nTo confirm, run:")
            print("  neut sense db delete --confirm")
            return
        ...
        print("Usage: neut sense db <command>")
        print()
        ...
        print()
        print("Quick Start:")
        print("  neut sense db up              # Start local PostgreSQL")
        print("  neut sense db migrate upgrade # Apply migrations")
        print("  neut sense db stats           # Check it's working")
        print()
        print("Configuration:")
...
def cmd_media(args):
    ...
    from tools.pipelines.sense.media_library import (
        MediaLibrary,
        SearchMode,
        format_duration,
    )
    ...
    if action == "search":
        ...
        from tools.pipelines.sense.media_library import SearchResult
        result = SearchResult(
            item=item,
            score=1.0,
        ...
        print("Usage: neut sense media <search|play|discuss|stats|index|list>")
        print()
        print("Commands:")
        ...
        print()
        print("Examples:")
        print("  neut sense media search heat exchangers --discuss")
        print("  neut sense media discuss abc123 --summary")
        print("  neut sense media discuss abc123 -q 'What was the decision?'")
...
def cmd_timestamps(args):
    ...
    raw_voice_dir = _AGENTS_DIR / "inbox" / "raw" / "voice"
...
def cmd_voice(args):
    ...
    """
    try:
        from tools.pipelines.sense.voice_id import VoiceProfileStore
    except ImportError:
    ...
    profiles = VoiceProfileStore(_AGENTS_DIR)
    all_profiles = profiles.list_profiles()

    # Load team roster for comparison
    people_file = _AGENTS_DIR / "config" / "people.md"
    team_size = 0
    if people_file.exists():
        content = people_file.read_text()
    ...
    if not all_profiles:
        ...
        print("  Run: neut sense corrections --guided --play")
