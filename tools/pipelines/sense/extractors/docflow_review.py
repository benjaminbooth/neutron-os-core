...
class DocFlowReviewExtractor:
    ...
    def __init__(
        ...
        self._agents_dir = Path(__file__).resolve().parent.parent.parent
        self._registry_path = registry_path or (
            self._agents_dir / "inbox" / "state" / "docflow_registry.json"
        )
        self._sync_state_path = self._agents_dir / "inbox" / "state" / "docflow_sync.json"
...
def register_prd(
    ...
    Usage:
        neut docflow register --prd reactor-ops-log \\
            --md docs/prd/reactor-ops-log-prd.md \\
