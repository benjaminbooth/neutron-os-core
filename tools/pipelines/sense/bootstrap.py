...
Usage:
    ...
    python -m tools.pipelines.sense.bootstrap

    # Non-interactive (CI/CD)
    python -m tools.pipelines.sense.bootstrap --non-interactive

    # Check prerequisites only
    python -m tools.pipelines.sense.bootstrap --check

    # Specific steps
    python -m tools.pipelines.sense.bootstrap --step k3d
    python -m tools.pipelines.sense.bootstrap --step postgres
    python -m tools.pipelines.sense.bootstrap --step migrate
...
class StepResult:
    ...
    def __str__(self) -> str:
        status = "✓" if self.success else ("○" if self.skipped else "✗")
...
"""
    ...
    def verify_installation(self) -> StepResult:
        ...
        try:
            from tools.pipelines.sense.migrations import verify_schema
            from tools.pipelines.sense.db_models import get_engine
