...
def _scan_extensions() -> dict[str, ToolDef]:
    ...
    for info in pkgutil.iter_modules([str(_EXT_DIR)]):
        ...
        mod_name = f"tools.agents.chat.tools_ext.{info.name}"
...
def execute_tool(name: str, params: dict[str, Any]) -> dict[str, Any]:
    ...
    if name in ext_tools:
        mod_name = None
        for info in pkgutil.iter_modules([str(_EXT_DIR)]):
            mod = sys.modules.get(f"tools.agents.chat.tools_ext.{info.name}")
    ...
    if name == "query_docs":
        ...
        from tools.pipelines.sense.cli import INBOX_RAW, INBOX_PROCESSED, DRAFTS_DIR
        counts: dict[str, int] = {}
        if INBOX_RAW.exists():
        ...
        from tools.pipelines.sense.cli import INBOX_RAW
