...
Six tools following the ToolDef + execute() pattern from
tools/agents/chat/tools_ext/read_file.py. The doctor uses these
directly (not via the chat tool registry) so they are self-contained.
