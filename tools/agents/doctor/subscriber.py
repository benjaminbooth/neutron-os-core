...
_DOCTOR_DIR = _AGENTS_DIR / "doctor"
_LOG_PATH = _AGENTS_DIR / "logs" / "cli_events.jsonl"
...
def _rollback_from_backup(files_changed: list[str]) -> None:
    ...
    from tools.agents.doctor.tools import rollback_file
...
def doctor_handler(topic: str, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        from tools.agents.doctor.agent import DoctorAgent
...
def review_handler(topic: str, data: dict[str, Any]) -> None:
    ...
    from tools.agents.doctor.reviewer import Reviewer
...
def commit_handler(topic: str, data: dict[str, Any]) -> None:
    ...
    # Try to commit now
    from tools.agents.doctor.tools import execute as exec_tool
