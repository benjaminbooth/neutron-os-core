...
Quick Start (local):
    ...
    # Use it
    from tools.agents.sense.pgvector_store import VectorDB
    db = VectorDB()
...
K3D_CONFIG_DIR = _AGENTS_DIR.parent.parent.parent / "infra" / "k3d"
...
class PgVectorStore:
    ...
    def chunks(self) -> list:
        ...
        from tools.agents.sense.signal_rag import SignalChunk
        return [
            SignalChunk(
                signal_id=r.id,
                text=r.text,
...
def k3d_up() -> bool:
    ...
    print()
    print("To use:")
    print("  from tools.agents.sense.pgvector_store import VectorDB")
    print("  db = VectorDB()")
    print()
