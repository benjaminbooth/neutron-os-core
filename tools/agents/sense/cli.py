...
def _ingest_prd_comments(correlator) -> list:
    ...
    from tools.agents.sense.extractors.prd_comments import PRDCommentsExtractor
...
def cmd_correct(args: argparse.Namespace) -> None:
    ...
    from tools.agents.sense.corrector import correct_transcript
...
def cmd_routes(args: argparse.Namespace) -> None:
    ...
    from tools.agents.sense.router import Router
...
def cmd_suggest(args: argparse.Namespace) -> None:
    ...
    from tools.agents.sense.smart_router import SmartRouter
...
def cmd_brief(args: argparse.Namespace) -> None:
    ...
    from tools.agents.sense.briefing import BriefingService
    ...
    if args.index:
        ...
        try:
            from tools.agents.sense.signal_rag import reindex_all_signals
...
def _tag_speaker_voice(audio_path: str, start_time: float, end_time: float, agents_dir: "Path") -> None:
    ...
    try:
        from tools.agents.sense.voice_id import VoiceProfileStore, SpeakerIdentifier
    except ImportError:
...
def cmd_corrections(args):
    ...
    from tools.agents.sense.correction_review import (
...
def cmd_sources(args):
    ...
    from tools.agents.sense.registry import (
        get_registry,
        print_sources_table,
        print_inbox_status,
    )
...
def cmd_providers(args):
    ...
    try:
        from tools.agents.sense.extractors.docflow_providers import (
            ProviderRegistry,
            sync_all_folders,
        )
    except ImportError as e:
    ...
    print("See: tools/agents/sense/extractors/DOCFLOW_PROVIDERS.md")


def cmd_db(args):
    """Manage vector database (PostgreSQL + pgvector via K3D)."""
    import os
    from tools.agents.sense.pgvector_store import (
        VectorDB,
        DEFAULT_LOCAL_URL,
        k3d_up,
        k3d_down,
    ...
    if action == "stats":
        ...
        from tools.agents.sense.signal_rag import SignalRAG, INDEX_PATH, EMBEDDINGS_PATH
        ...
        try:
            ...
            from tools.agents.sense.signal_rag import VectorStore
...
def cmd_media(args):
    ...
    from tools.agents.sense.media_library import (
        MediaLibrary,
        SearchMode,
        format_duration,
    )
    ...
    if action == "search":
        ...
        from tools.agents.sense.media_library import SearchResult
        result = SearchResult(
            item=item,
            score=1.0,
...
def cmd_voice(args):
    ...
    """
    try:
        from tools.agents.sense.voice_id import VoiceProfileStore
    except ImportError:
