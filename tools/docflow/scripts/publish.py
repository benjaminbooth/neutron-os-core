...
from tools.docflow.cleanup import apply_cleanup
from tools.docflow.validation import DocumentValidator
from tools.docflow.state import StateStore, DocumentState, PublicationRecord
...
def main():
    ...
    state_path = repo_root / ".doc-state.json"
    ...
    try:
        # Run pandoc from the docs/prd directory so relative image paths resolve
        cmd = [
            pandoc,
            "-f", "markdown",
            "-t", "docx",
