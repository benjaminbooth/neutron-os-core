"""SharePoint provider for DocFlow.

Features:
- Interactive device-code auth via MSAL (requires `DOCFLOW_CLIENT_ID` and `DOCFLOW_TENANT_ID` or .docflow/config.json)
...
Usage:
    ...
    p.pull(sharepoint_url, Path('docs/prd/mydoc.md'))
...
CONFIG_PATH = Path('.docflow/config.json')
TOKEN_CACHE_PATH = Path('.docflow/token_cache.json')
...
class SharePointProvider:
    ...
    def __post_init__(self):
        ...
        if not self.client_id or not self.tenant_id:
            raise RuntimeError('Missing DOCFLOW_CLIENT_ID or DOCFLOW_TENANT_ID. Add to .docflow/config.json or env vars.')
