...
Configuration:
- Environment variables OR .docflow/config.json:
  {
    "box": {
      "client_id": "...",
      "client_secret": "...",
...
Usage:
    from tools.docflow.providers.box import BoxProvider
    provider = BoxProvider()

    # Upload
    result = provider.upload(
        local_path=Path("docs/prd/my-prd.docx"),
    ...
    def __post_init__(self):
        ...
        if not self.client_id or not self.client_secret:
            raise RuntimeError(
                ...
                "Add to .docflow/config.json or set environment variables."
            )
...
- **Token caching**: Store refresh tokens in .docflow/token_cache.json
