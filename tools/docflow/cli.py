...
def cmd_scan(args: argparse.Namespace) -> None:
    ...
    # Default to docs/prd and docs/specs if no folders specified
    if args.folders:
...
def cmd_onboard(args: argparse.Namespace) -> None:
    ...
    if args.folder:
        ...
        # Infer from file path or default to docs/prd
        file_path = Path(args.file).resolve()
        if "specs" in str(file_path):
            manifest_dir = engine.config.repo_root / "docs" / "specs"
        else:
            manifest_dir = engine.config.repo_root / "docs" / "prd"
...
def get_parser() -> argparse.ArgumentParser:
    ...
    scan_parser.add_argument("folders", nargs="*", help="Folders to scan (default: docs/prd docs/specs)")
