...
> index with context.
>
> For strategic context, see the [Executive PRD](../requirements/prd_neutron-os-executive.md).
...
|-----------|------|--------|-----------|
| **LLM Gateway** | [Model Routing Spec](neutron-os-model-routing-spec.md) | ✅ Phase 1+2a shipped | `infra/gateway.py`, `infra/router.py` |
| **Export Control Router** | [Model Routing Spec §4](neutron-os-model-routing-spec.md) | ✅ Shipped | Keyword + Ollama SLM + sensitivity knobs |
| **Hybrid State Store** | [State Management Spec](agent-state-management-spec.md) | ✅ Shipped | `infra/state.py` (file + PostgreSQL backends) |
| **Routing Audit Log** | [Model Routing Spec §11](neutron-os-model-routing-spec.md) | ✅ Shipped | `infra/routing_audit.py` → JSONL |
| **Connections** | [Connections Spec](neutron-os-connections-spec.md) | 📋 Spec'd | Credential resolution, `neut connect` |
| **Settings** | [Model Routing Spec §6](neutron-os-model-routing-spec.md) | ✅ Shipped | `settings/store.py` (global + project scope) |

### Agents

| Agent | CLI Noun | Spec | Status | Description |
|-------|----------|------|--------|-------------|
| **Signal** | `neut signal` | [Agent Architecture](neutron-os-agent-architecture.md) | ✅ Shipped | Signal ingestion: voice, Teams, GitLab, GitHub, freetext → extractors → correlator → synthesizer |
| **Chat** | `neut chat` | [Agent Architecture](neutron-os-agent-architecture.md) | ✅ Shipped | Interactive LLM with tool use, RAG, per-turn routing, TUI + REPL |
| **M-O** | `neut mo` | [State Management Spec](agent-state-management-spec.md) | ✅ Shipped | Resource steward: scratch lifecycle, retention enforcement, vitals |
...
|------|----------|------|--------|-------------|
| **Publisher** | `neut pub` | [Publisher Spec](neutron-os-publisher-spec.md) | ✅ Shipped | Document lifecycle: markdown → generate → publish to 19 endpoints |
| **RAG** | `neut rag` | [RAG Architecture](neutron-os-rag-architecture-spec.md) | ✅ Shipped | Three-tier corpus (community/org/personal), pgvector, EC-compliant |
...
**Authoritative spec:** [Data Architecture Spec](data-architecture-spec.md)
...
**Retention:** Configurable via `runtime/config/retention.yaml`. M-O enforces automatically. See [State Management PRD](../requirements/prd_agent-state-management.md).
...
**Authoritative specs:**
- [Security & Access Control PRD](../requirements/prd_security-access-control.md)
- [Model Routing Spec §7-8-10](neutron-os-model-routing-spec.md)
- [Connections Spec §8](neutron-os-connections-spec.md)
...
| **TACC endpoint** | vLLM on TACC (Apptainer) | 📋 Proposed ([Routing Spec §9](neutron-os-model-routing-spec.md)) |
...
- Declare connections in TOML → platform handles credentials ([Connections Spec](neutron-os-connections-spec.md))
- Use `get_credential()` for auth → never hardcode tokens
- Use `LockedJsonFile` or `get_state_store()` for state → never raw `json.loads` ([State Spec](agent-state-management-spec.md))

---

## Integration Points

**Authoritative spec:** [Connections Spec](neutron-os-connections-spec.md)
...
**Authoritative spec:** [Digital Twin Architecture Spec](digital-twin-architecture-spec.md)
...
| **WASM runtime** | Sandboxed execution of surrogate models | 📋 Spec'd ([ADR-008](../requirements/adr_008-wasm-extension-runtime.md)) |
...
| ADR | Decision | Status |
|-----|----------|--------|
| [ADR-001](../requirements/adr_001-polyglot-monorepo-bazel.md) | Monorepo (Bazel dropped → pure Python) | Accepted (modified) |
| [ADR-002](../requirements/adr_002-hyperledger-fabric-multi-facility.md) | Hyperledger Fabric for tamper-proof audit | Accepted |
| [ADR-003](../requirements/adr_003-lakehouse-iceberg-duckdb-superset.md) | Iceberg + DuckDB lakehouse | Accepted |
| [ADR-004](../requirements/adr_004-infrastructure-terraform-k8s-helm.md) | Terraform + K8S + Helm | Accepted |
| [ADR-005](../requirements/adr_005-meeting-intake-pipeline.md) | Meeting intake → Signal pipeline | Accepted |
| [ADR-006](../requirements/adr_006-mcp-server-agentic-access.md) | MCP server for IDE integration | Accepted |
