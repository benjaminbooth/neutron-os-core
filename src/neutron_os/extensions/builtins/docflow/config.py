...
Loads configuration via a multi-tier discovery hierarchy:
  ...
  3. .doc-workflow.yaml in current directory
...
def _discover_config_path() -> Path | None:
    ...
    # Tier 2: .neut/docflow/workflow.yaml in project root
    candidate = PROJECT_ROOT / ".neut" / "docflow" / "workflow.yaml"
    if candidate.exists():
        return candidate
    ...
    # Tier 5: legacy .doc-workflow.yaml in project root
    candidate = PROJECT_ROOT / ".doc-workflow.yaml"
    if candidate.exists():
        return candidate
...
def load_config(config_path: Path | None = None) -> DocFlowConfig:
    """Load DocFlow configuration from YAML file.
