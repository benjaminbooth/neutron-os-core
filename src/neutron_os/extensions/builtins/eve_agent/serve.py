"""HTTP ingestion server for neut signal.
...
Usage:
    neut signal serve [--port 8765] [--host 0.0.0.0] [--process] [--webhook URL]
...
def _generate_shortcut_plist(server_url: str, share_sheet: bool = False) -> bytes:
    ...
    actions.append({
        ...
        "WFWorkflowActionParameters": {
            "Text": {
                "Value": {"string": "✓ Sent to neut signal"},
                "WFSerializationType": "WFTextTokenString"
            }
        }
    })
...
def run_server(
    ...
    print(NEWT_BANNER)
    print("neut signal serve — inbox ingestion server")
    print("─" * 50)
    print()
    if urls:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="neut signal inbox server")
