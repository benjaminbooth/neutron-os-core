...
Usage:
    ...
    python -m tools.extensions.builtins.eve_agent.bootstrap

    # Non-interactive (CI/CD)
    python -m tools.extensions.builtins.eve_agent.bootstrap --non-interactive

    # Check prerequisites only
    python -m tools.extensions.builtins.eve_agent.bootstrap --check

    # Specific steps
    python -m tools.extensions.builtins.eve_agent.bootstrap --step k3d
    python -m tools.extensions.builtins.eve_agent.bootstrap --step postgres
    python -m tools.extensions.builtins.eve_agent.bootstrap --step migrate

CLI:
    neut signal bootstrap
    neut signal bootstrap --check
"""
...
"""
    ...
    def run(
        ...
        if all(r.success for r in self.results):
            ...
            self._log("\nNext steps:")
            self._log("  neut signal db stats     # Check database")
            self._log("  neut signal ingest       # Start ingesting signals")
        else:
    ...
    def check_only(self) -> list[StepResult]:
        ...
        self._log(f"\n{'✅ Ready' if all_ok else '⚠️  Setup needed: neut signal bootstrap'}")
